...
| `thinkingLevel` | **Real thinking override** for that call: `off`, `minimal`, `low`, `medium`, `high`, `xhigh`, or `max`. Omit it to inherit the parent session. Invalid values are logged and ignored. Built-in `deep-research` / `code-review` pin every subagent to `high`; override on demand in a custom script. |
| `agentType` | **Real named agent definition.** Resolved from `~/.pi/agent/agents/*.md` and `<cwd>/.pi/agents/*.md` (project overrides user): Markdown frontmatter (`name`, `description`, `model`, `thinking` / `thinkingLevel`, `tools`) + body as the role system prompt. Applies the role prompt, optional tool allowlist, and optional model/thinking. Explicit `opts.thinkingLevel` wins over the agent type. |
...
Per-agent options are part of the journal cache key (plain calls keep their old v1 keys, so existing journals still replay). Explicit `opts.thinkingLevel` is part of the key; inherited parent thinking is not.
